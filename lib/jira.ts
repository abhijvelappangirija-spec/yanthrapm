...
export async function createJiraStory(
  ...
  const storyPointsFieldId = process.env.JIRA_STORY_POINTS_FIELD_ID || 'customfield_10026'
  if (story.points > 0) {
  ...
  if (epicKey) {
    const epicLinkFieldId = process.env.JIRA_EPIC_LINK_FIELD_ID || 'customfield_10014'
